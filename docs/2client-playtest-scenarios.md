...
2. 자동 시나리오 실행(권장): `npm run playtest:2client`
3. 클라이언트 A: 일반 브라우저 창
4. 클라이언트 B: 시크릿 창(또는 다른 브라우저)
5. 둘 다 같은 URL 접속(기본 `http://localhost:5173`)
6. 둘 다 ONLINE 탭에서 GLOBAL 로비 진입

공통 체크:

1. 입장 시 서버가 자동으로 팀을 배정해야 함
2. 필요하면 ALPHA/BRAVO 버튼으로 팀 재선택 가능해야 함
3. 매치는 GLOBAL 방 참가 상태면 시작 가능해야 함

## Scenario 1: 재접속 (Reconnect)

절차:

1. A/B 접속 후 자동 팀 배정 확인(필요 시 수동 팀 조정) 후 매치 시작
...
3. B 자동 팀 배정 확인(필요 시 수동 팀 조정) 후 매치 시작 이벤트 수신
